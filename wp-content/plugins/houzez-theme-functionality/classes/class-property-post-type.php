...
class Houzez_Post_Type_Property {
    ...
    public static function init() {
        ...
        add_action('admin_init', array( __CLASS__, 'houzez_sold_listing' ));
    ...
    public static function custom_columns() {
        ...
        $columns["thumbnail"] = __( 'Thumbnail','houzez-theme-functionality' );
        ...
        $columns["id"] = __( 'Property ID','houzez-theme-functionality' );
            ...
            case 'houzez_actions':
                ...
                if ( in_array( $post->post_status, array( 'publish' ) ) && in_array( 'administrator', (array) $user->roles ) ) {
                    $admin_actions['mark_sold']   = array(
                        ...
                        'url'     =>  wp_nonce_url( add_query_arg( 'sold_listing', $post->ID ), 'sold_listing' )
                    );
                }
    ...
    {
        if (!empty($_GET['sold_listing']) && wp_verify_nonce($_REQUEST['_wpnonce'], 'sold_listing') && current_user_can('publish_post', $_GET['sold_listing'])) {
            $post_id = absint($_GET['sold_listing']);
            $listing_data = array(
                'ID' => $post_id,
            ...
            wp_redirect(remove_query_arg('sold_listing', add_query_arg('sold_listing', $post_id, admin_url('edit.php?post_type=property'))));
            exit;
        }
    }
